function Connect(a, b) {
    this.map = a, 
    this.svg = b, 
    this.path_id = Math.random().toString(36).substring(7),
    ...
    this.roadColor = "white",// transparent
    ...
    this.init = function(a, b) {
        ...
        var c = Math.random() < .5 ? -1 : 1,
            d = (a.lat + b.lat) / 2,
            e = (a.lng + b.lng) / 2;
        this.midPoint.lat = (a.lat + b.lat) / 2 + Math.random() * d * 0.85 * c, 
        this.midPoint.lng = (a.lng + b.lng) / 2 + Math.random() * e * 0.85 * c, 
        ...
        this.clipPath = this.group.append("defs").append("clipPath").attr("id", this.path_id).attr("clip-rule", "nonzero");
        ...
        this.group_road.attr("clip-path", "url(#" + this.path_id + ")"), 
        this.road = this.group_road.append("path").datum(this.road_points).attr("stroke", this.roadColor).attr("stroke-width", 1.5).attr("stroke-linecap", "1").attr("fill", "none").attr("d", d3.line().curve(d3.curveBundle.beta(.5))), 
        this.bp_circle = this.group.append("circle").attr("fill", this.beginColor).attr("class", 'fade'),
        this.group.append("circle")
        .attr("class", 'fade')
        .attr("cx", this.bp_px.x)
        .attr("cy", this.bp_px.y)
        .attr("r", 8)
        .attr("fill", 'transparent')
        .attr("stroke", this.beginColor)
        , 
        this.ep_circle = this.group.append("rect")
        .attr("class", 'fade')
        .attr("x", this.ep_px.x)
        .attr("y", this.ep_px.y)
        .attr("width", 6)
        .attr("height", 6)
        .attr("fill", this.endColor),
        this.group.append("rect")
        .attr("class", 'fade')
        .attr("x", this.ep_px.x - 3)
        .attr("y", this.ep_px.y - 3)
        .attr("width", 12)
        .attr("height", 12)
        .attr("fill", 'transparent')
        .attr("stroke", this.endColor)
        , 
        this.load_plane();
    ...
    this.delete = function() {
        this.ep_circle = this.group.append("circle")
        .attr("cx", this.ep_px.x + 3)
        .attr("cy", this.ep_px.y + 3)
        .attr("r", 1)
        .attr("fill", this.endColor)
        .attr("stroke", this.endColor)
        ...
        this.road.transition().duration(1000).style("opacity", "0.0").attr("stroke-width", 0).remove(), 
        this.group.transition().delay(1500).style("opacity", "0.0").remove()
    }, 
    this.d_plane = "M32,0C14.327,0,0,14.327,0,32c0,17.673,14.327,32,32,32c17.673,0,32-14.327,32-32C64,14.327,49.673,0,32,0z M32,61C15.983,61,3,48.016,3,32C3,15.983,15.983,3,32,3s29,12.983,29,29C61,48.016,48.017,61,32,61z M42.25,32 c0,5.661-4.589,10.25-10.25,10.25S21.75,37.661,21.75,32S26.339,21.75,32,21.75S42.25,26.339,42.25,32z"
}